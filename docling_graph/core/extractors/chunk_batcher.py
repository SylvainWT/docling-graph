...
class ChunkBatcher:
    ...
    def __init__(
        ...
        self.available_tokens = (
            context_limit - system_prompt_tokens - response_buffer_tokens
        )
    ...
    def batch_chunks(
        ...
        for chunk_idx, chunk_text in enumerate(chunks):
            ...
            if (
                ...
                and potential_total > self.available_tokens
            ):
                # Start new batch
                batches.append(
                    ChunkBatch(
    ...
    def _merge_undersized_batches(
        ...
        while i < len(batches):
            ...
            while j < len(batches):
                ...
                potential_total = (
                    combined_tokens + next_batch.total_tokens
                )
    ...
    def _log_batching_summary(
        ...
        avg_utilization = (
            total_tokens / (len(batches) * self.available_tokens) * 100
            if batches
            else 0
        )
        ...
        rich_print(
            ...
            f"  • Batches created: [yellow]{len(batches)}[/yellow] "
            f"([green]-{reduction:.0f}%[/green] API calls)\n"
        ...
        for batch in batches:
            utilization = (
                batch.total_tokens / self.available_tokens * 100
            )
            rich_print(
